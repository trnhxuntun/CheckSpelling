...
class DatnGui1App:
    def __init__(self, master=None, sc=None):
        ...
        self.__tkvar3 = tk.StringVar(value='Manage')
        __values = ['New', 'Open', 'Save', 'Save as']
        ...
        __values = ['Manage Autofix', 'Manage Dict']
        ...
        self.lb_ErrorNumber.configure(text='Found 0 error(s)')
        ...
        self.txt_file.configure(undo=True, wrap=tk.WORD)
        ...
        self.btn_Check.configure(text='Check (F5)')
    ...
    def opt_managepicked(self, option):
        if option == 'Manage Autofix':
            AutoFixGUI(tk.Toplevel(self.master), self)
        elif option == 'Manage Dict':
    ...
    def run_check_threaded(self, file=None):
        ...
        for i in range(len(self.autofix_inp)):
            ...
            #self.sc.content = re.sub('\b' + self.autofix_inp[i] + '\b', self.autofix_onp[i], self.sc.content)
        ...
        if self.sc.content.strip() != '' or True:
            ...
            for err_word in err_punc_list:
                soloi+=1
                self.txt_file.tag_add(self.marked_tag, self.to_coord(err_word[0]+1, err_word[1]), self.to_coord(err_word[0]+1, err_word[1] + len(err_word[2])))
            self.txt_file.tag_config(self.marked_tag, background='black', foreground='white', wrap=tk.WORD)
            ...
            self.lb_ErrorNumber.configure(text='Found %s error(s)' % soloi)
...
if __name__ == '__main__':
    ...
    root.title('Kiem tra loi chinh ta')
